...
//
//Trajectory TrajectoryGenerator::getTrajectory(double target_d, double target_s, Car &car, Trajectory &previous_path) {
//    double car_s = car.s;
...
Trajectory TrajectoryGenerator::getTrajectory(int lane, double targetVelocity, Car &car, Trajectory &previous_path) {
    ...
    for (int i = 0; i < prev_size; i++) {
        ...
        double theta = ref_yaw;
        if (i>0){
            theta = atan2(previous_path.y_values[i] - previous_path.y_values[i-1], previous_path.x_values[i] - previous_path.x_values[i-1]);
        }
      //  auto frenet = map.getFrenet(previous_path.x_values[i], previous_path.y_values[i], car.s);
      //  next_s_vals.push_back(frenet[0]);
     //   next_d_vals.push_back(frenet[1]);
    ...
    std::string traj_d =" trajectory d: ";
    for (int i = 0; i < Helper::LookForward - prev_size; i++) {
        ...
        double theta = ref_yaw;
        if (i > 0){
            theta = atan2(y_point  - next_y_vals[i-1], x_point - next_x_vals[i-1]);
        }
      //  auto frenet = map.getFrenet(x_point, y_point, theta);
      //  next_s_vals.push_back(frenet[0]);
     //   next_d_vals.push_back(frenet[1]);
        theta_vals.push_back(theta);
     ...
     return { next_x_vals, next_y_vals, next_s_vals, next_d_vals, theta_vals, lane};
}
